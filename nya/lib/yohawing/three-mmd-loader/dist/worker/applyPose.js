...
export function createMmdRuntimePoseApplyScratch(mesh) {
    ...
    for (let index = 0; index < bones.length; index += 1) {
        ...
        parentIndices[index] = bones[index]?.parent
            ? bones.indexOf(bones[index]?.parent)
            : -1;
    }
    return {
        worldMatrices,
...
export function applyMmdRuntimePoseToMesh(pose, mesh, scratch) {
    const bones = mesh.skeleton.bones;
    if (pose.worldMatricesColumnMajor.length !== bones.length * 16 ||
