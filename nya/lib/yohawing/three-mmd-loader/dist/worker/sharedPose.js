import { MMD_RUNTIME_POSE_PROTOCOL_VERSION } from "./protocol.js";
const sharedPoseFree = 0;
const sharedPoseWriting = 1;
const sharedPoseReady = 2;
const sharedPoseReading = 3;
export function createMmdRuntimeSharedPoseSlots(boneCount, morphCount, capacity = 3) {
    ...
    for (let index = 0; index < capacity; index += 1) {
        slots[index] = {
            ...
            worldMatricesColumnMajor: new Float32Array(new SharedArrayBuffer(Float32Array.BYTES_PER_ELEMENT * boneCount * 16)),
            morphWeights: new Float32Array(new SharedArrayBuffer(Float32Array.BYTES_PER_ELEMENT * morphCount))
        };
    }
    return slots;
}
/** Claims one free slot for a worker-side write without blocking. */
export function acquireMmdRuntimeSharedPoseWriteSlot(slots) {
...
export function publishMmdRuntimeSharedPose(slot, source) {
...
export function readMmdRuntimeSharedPoseInto(slot, target) {
    if (Atomics.compareExchange(slot.control, 0, sharedPoseReady, sharedPoseReading) !== sharedPoseReady) {
        return undefined;
    }
...
export function releaseMmdRuntimeSharedPoseReadSlot(slot) {
    if (Atomics.compareExchange(slot.control, 0, sharedPoseReading, sharedPoseFree) !== sharedPoseReading) {
        throw new Error("MMD runtime shared pose slot is not owned for reading");
    }
    Atomics.notify(slot.control, 0);
}
export function resetMmdRuntimeSharedPoseSlot(slot) {
    Atomics.store(slot.control, 0, sharedPoseFree);
    Atomics.store(slot.control, 1, 0);
    Atomics.store(slot.control, 2, 0);
}
export function createMmdRuntimeSharedPoseReadBuffer(boneCount, morphCount) {
    assertCount(boneCount, "bone");
    assertCount(morphCount, "morph");
    return {
...
function assertPayloadLengths(slot, pose) {
    if (slot.worldMatricesColumnMajor.length !== pose.worldMatricesColumnMajor.length ||
