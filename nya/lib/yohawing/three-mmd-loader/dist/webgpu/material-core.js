...
class MmdTslLightingModel extends THREE.LightingModel {
    finish(builder) {
        const { outgoingLight } = builder.context;
        const typedOutgoingLight = outgoingLight;
...
export function createMmdTslToonMaterial(options = {}) {
    ...
    if (options.diffuseMap) {
        material.opacityNode = TSL.materialOpacity.mul(TSL.texture(options.diffuseMap).sample(TSL.uv()).a);
    }
 ...
 */
function applySphereComposite(base, sphereMode, compositeSphere, sphereTextureFactor) {
...
export function createMmdTslBaseColorNode(options = {}) {
    ...
    const lightDirectionView = TSL.normalize(cameraViewMatrix.transformDirection(lightDirectionNode));
    ...
    const toonCoordinate = TSL.clamp(signedDot.mul(0.5).add(toonCoordinateOffset), 0, 1);
    ...
    const compositeDiffuseTexture = options.gammaSpaceComposite === true && options.diffuseMap
        ? TSL.sRGBTransferOETF(diffuseTexture)
        : diffuseTexture;
    const compositeSphere = options.gammaSpaceComposite === true && options.sphereMap
        ? TSL.sRGBTransferOETF(sampledSphere)
        : sampledSphere;
    ...
    const legacyColor = options.legacySrgbFramebuffer === true
        ? gammaComposite
        : TSL.sRGBTransferEOTF(gammaComposite);
    ...
    const dedicatedVis = dedicatedInFrustum.select(options.toonMap ? TSL.min(dedicatedNLGrade, dedicatedShadowFactor) : dedicatedShadowFactor, TSL.float(1));
    ...
    const dedicatedSelfShadowToon = options.toonMap
        ? TSL.mix(TSL.vec3(1, 1, 1), TSL.texture(options.toonMap).sample(TSL.vec2(0, 0)).rgb.mul(toonTextureFactor.rgb), toonTextureFactor.a)
    ...
    const dedicatedSpecularComposite = TSL.pow(TSL.max(0, TSL.dot(halfDirection, normalView)), specularPower)
        .mul(specular)
        .mul(lightColor)
        .mul(dedicatedVis)
        .mul(specularGate);
    ...
    const dedicatedGammaComposite = TSL.clamp(TSL.mix(dedicatedShadowColor, dedicatedLitNoSpec, dedicatedVis).add(dedicatedSpecularComposite), 0, 1);
    const dedicatedColor = options.legacySrgbFramebuffer === true
        ? dedicatedGammaComposite
        : TSL.sRGBTransferEOTF(dedicatedGammaComposite);
    return TSL.mix(legacyColor, dedicatedColor, uniforms.dedicatedShadowEnabled);
}
export function syncMmdTslMaterialState(material, state) {
    ...
    uniforms.textureFactor.set(state.textureFactor[0], state.textureFactor[1], state.textureFactor[2], state.textureFactor[3]);
    uniforms.sphereTextureFactor.set(state.sphereTextureFactor[0], state.sphereTextureFactor[1], state.sphereTextureFactor[2], state.sphereTextureFactor[3]);
    uniforms.toonTextureFactor.set(state.toonTextureFactor[0], state.toonTextureFactor[1], state.toonTextureFactor[2], state.toonTextureFactor[3]);
    ...
    if (material.visible !== previousVisible ||
...
function createMmdTslMaterialUniforms(options) {
    return {
        ...
        specularPower: TSL.uniform(options.specularPower ?? 0, "float"),
        toonCoordinateOffset: TSL.uniform(options.toonCoordinateOffset ?? MMD_TSL_DEFAULT_TOON_COORD_OFFSET, "float"),
        ...
        dedicatedShadowEnabled: TSL.uniform(0, "float")
    };
}
function vectorFromTuple(value) {
    return new THREE.Vector3(value[0], value[1], value[2]);
}
function vector4FromTuple(value) {
