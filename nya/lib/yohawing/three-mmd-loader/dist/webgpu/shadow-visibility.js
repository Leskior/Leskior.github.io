 ...
 */
export function createMmdTslShadowVisibilityNode(light, depthTexture, options = {}) {
    ...
    const shadowMode = uniform(1, "float");
    const shadowIntensity = saturate(reference("intensity", "float", light.shadow).setGroup(renderGroup));
    const bias = reference("bias", "float", light.shadow).setGroup(renderGroup);
    const normalBias = reference("normalBias", "float", light.shadow).setGroup(renderGroup);
    const visibilityNode = Fn(() => {
        const shadowPosition = shadowMatrix.mul(vec4(positionWorld.add(normalWorld.mul(normalBias)), 1));
        ...
        const shadowCoord = vec3(projected.x, projected.y.oneMinus(), biasedZ);
        ...
        const rampScale = shadowMode.equal(2).select(shadowCoord.y.mul(8000), float(1500));
        ...
        const visibility = float(1).sub(saturate(depthDelta.mul(rampScale).sub(0.3)).mul(shadowIntensity));
        ...
        return inFrustum.select(visibility, float(-1));
    })();
