...
const shadowCasterStates = new WeakMap();
/**
 ...
 */
export function createMmdTslShadowCaster(mesh, options = {}) {
    const existing = shadowCasterStates.get(mesh);
    if (existing) {
        return existing.proxy;
    }
    const sourceMaterials = Array.isArray(mesh.material) ? mesh.material : [mesh.material];
    const opaqueBuckets = new Map();
    const alphaBuckets = new Map();
...
export function disposeMmdTslShadowCaster(mesh) {
    return releaseMmdTslShadowCaster(mesh, true);
}
function releaseMmdTslShadowCaster(mesh, restoreSource) {
...
function createShadowCasterGeometry(source, buckets) {
...
function readSourceGroups(geometry) {
...
function createOpaqueShadowMaterial(source) {
...
function createAlphaTestShadowMaterial(source, diffuseMap) {
...
function shadowSideKey(material) {
...
function readDiffuseMap(material) {
    const references = material.userData.mmdTslTextureReferences;
