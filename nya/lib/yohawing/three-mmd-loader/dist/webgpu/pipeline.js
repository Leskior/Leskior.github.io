...
import { replaceMmdModelMaterialsWithTsl } from "./material-assembly.js";
import { syncMmdTslMaterialState } from "./material-core.js";
import { createMmdTslSelfShadowPass } from "./self-shadow-pass.js";
import { computeMmdTslSparsePositionMorphs, disposeMmdTslSparsePositionMorphs, enableMmdTslSparsePositionMorphs } from "./sparse-morph-runtime.js";
import { createMmdTslShadowCaster, disposeMmdTslShadowCaster } from "./shadow-caster.js";
/**
 ...
 */
export function createModelLoadOptions(overrides = {}) {
    const defaults = {
        frustumCulled: false,
        morphSplit: false,
 ...
 */
export async function createMmdTslPipeline(renderer, options = {}) {
    ...
    if (candidate.isWebGPURenderer !== true ||
        ...
        throw new Error("MMD_TSL_PIPELINE_NATIVE_WEBGPU_REQUIRED: renderer initialized with a non-WebGPU backend");
    }
    return createInitializedPipeline(renderer, options);
}
function createInitializedPipeline(renderer, options) {
    const attached = [];
    const attachedByMesh = new WeakMap();
    ...
    let selfShadowMode = options.selfShadowMode ?? 1;
    let disposed = false;
    const pipeline = {
        renderer,
        get light() {
        ...
        attach(model, attachOptions = {}) {
            ...
            if (needsSelfShadowVisibility && !light) {
                throw new Error("MMD_TSL_PIPELINE_SELF_SHADOW_LIGHT_REQUIRED: provide options.light before attaching a receiver model");
            }
            ...
            for (let index = 0; index < materials.length; index += 1) {
                ...
                const candidateUniforms = material.userData.mmdTslMaterialUniforms;
        ...
        prepareRender(_scene) {
            ...
            for (let index = 0; index < attached.length; index += 1) {
                ...
                if (state.sparseMorphs) {
                    computeMmdTslSparsePositionMorphs(renderer, state.model.mesh);
                }
    ...
    function syncLightUniforms() {
        ...
        for (let modelIndex = 0; modelIndex < attached.length; modelIndex += 1) {
            ...
            for (let materialIndex = 0; materialIndex < state.uniforms.length; materialIndex += 1) {
                ...
                if (light.visible) {
                    uniforms.lightColor.set(light.color.r * light.intensity, light.color.g * light.intensity, light.color.b * light.intensity);
                }
                else {
    ...
    function syncMaterialStates(state) {
        for (let materialIndex = 0; materialIndex < state.materials.length; materialIndex += 1) {
            ...
            if (outline) {
                ...
                const materialState = sourceMaterial?.userData.mmdMaterialState;
                if (materialState) {
    ...
    function syncReceiverVisibility(state) {
...
function hasSelfShadowReceiver(mesh) {
    const materials = normalizeMaterials(mesh.material);
    for (let index = 0; index < materials.length; index += 1) {
        const flags = materials[index].userData.mmdMaterial;
...
function syncOutlineMaterialState(material, state, metadata) {
    ...
    metadata.uniforms?.color?.set(clampColor(state.edgeColor[0]), clampColor(state.edgeColor[1]), clampColor(state.edgeColor[2]));
