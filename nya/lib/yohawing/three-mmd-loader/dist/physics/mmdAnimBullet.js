...
class MmdAnimBulletPhysicsBackendImpl {
    module;
    options;
    name = "mmd-anim-bullet";
    disabled = false;
    world = 0;
    disposedState = false;
    modelKey;
    ...
    scratchLocal = new Matrix4();
    scratchScale = new Vector3(1, 1, 1);
    scratchTranslation = new Vector3();
    scratchRotation = new Quaternion();
    scratchPosition = new Float32Array(3);
    scratchQuaternion = new Float32Array(4);
    outWorldPointer;
    outIndexPointer;
    ...
    step(context) {
        ...
        if (seedOnly) {
            ...
            if (inputWorld)
                this.feedBodies(inputWorld, undefined, true, true);
    ...
    debugPhysicsContacts() {
        ...
        if (collect(this.world, this.contactBufferPointer, this.contactBufferCapacity, this.contactCountPointer) !== 0)
            return [];
        this.module.refreshMemoryViews?.();
    ...
    ensureModel(context) {
        ...
        if (this.modelKey === key)
            return true;
        if (this.modelKey !== undefined || this.bodyCount > 0)
            this.recreateWorld();
        ...
        try {
            for (const body of bodies) {
                ...
                if (this.module._mmd_anim_bullet_world_add_rigidbody(this.world, bodyPtr, this.outIndexPointer) !== 0)
                    return false;
                this.module.refreshMemoryViews?.();
                this.bodyBoneIndices.push(body.boneIndex ?? -1);
                this.bodyModes.push(body.motionType);
                const bodyBind = new Matrix4().compose(new Vector3(bodyPosition[0] ?? 0, bodyPosition[1] ?? 0, bodyPosition[2] ?? 0), new Quaternion(...(body.localRotation ?? [0, 0, 0, 1])), this.scratchScale);
            ...
            try {
                for (const joint of joints) {
                    ...
                    if (this.module._mmd_anim_bullet_world_add_6dof_spring_joint(this.world, jointPtr, this.outIndexPointer) !== 0)
                        return false;
                }
            }
            finally {
                this.module._free(jointPtr);
            }
            this.modelKey = key;
    ...
    feedBodies(input, toggles, includeDynamic, includeDynamicWithBone) {
        const heap = this.module.HEAPF32;
        if (!heap)
            return;
        for (let i = 0; i < this.bodyCount; i += 1) {
            ...
            if (!bodyFromBone)
                continue;
            this.scratchBone.fromArray(input, boneIndex * 16);
            ...
            this.scratchPosition[0] = this.scratchTranslation.x;
            this.scratchPosition[1] = this.scratchTranslation.y;
            this.scratchPosition[2] = this.scratchTranslation.z;
            this.scratchQuaternion[0] = this.scratchRotation.x;
            this.scratchQuaternion[1] = this.scratchRotation.y;
            this.scratchQuaternion[2] = this.scratchRotation.z;
            this.scratchQuaternion[3] = this.scratchRotation.w;
    ...
    copyDynamicOutputs(context, toggles) {
        ...
        for (let i = 0; i < this.bodyCount; i += 1) {
            ...
            if (boneIndex < 0 ||
                ...
                continue;
            if (this.module._mmd_anim_bullet_world_get_rigidbody_transform(this.world, i, this.transformPositionPointer, this.transformRotationPointer) !== 0)
                continue;
            const heap = this.module.HEAPF32;
            if (!heap)
                continue;
        ...
        for (let boneIndex = 0; boneIndex < this.targetWorldUpdated.length; boneIndex += 1) {
            if (this.targetWorldUpdated[boneIndex] === 0)
                continue;
            const worldOffset = boneIndex * 16;
            ...
            if (parentIndex >= 0 && parentIndex < this.targetWorldUpdated.length && this.targetWorldUpdated[parentIndex] !== 0) {
                ...
                this.scratchParent.fromArray(inputWorld, parentIndex * 16);
            }
            else {
            ...
            if (output.updatedBoneIndices)
                writeIndex(output.updatedBoneIndices, updated, boneIndex);
            updated += 1;
        }
        return updated;
    }
}
function writeVec3(target, offset, values) { target[offset] = values[0] ?? 0; target[offset + 1] = values[1] ?? 0; target[offset + 2] = values[2] ?? 0; }
function writeNumeric3(target, offset, x, y, z) {
...
function writeNumeric4(target, offset, x, y, z, w) {
    if (offset < target.length)
        target[offset] = x;
    if (offset + 1 < target.length)
        target[offset + 1] = y;
...
function writeNumeric16(target, offset, values) {
    for (let index = 0; index < 16 && offset + index < target.length; index += 1)
        target[offset + index] = values[index] ?? 0;
}
function writeIndex(target, offset, value) {
