...
export function parsePmxMetadata(input) {
    return parsePmx(input).metadata;
}
export function parsePmxSectionInventory(input) {
    return parsePmx(input).inventory;
}
function parsePmx(input) {
    ...
    readFixedByteSection(reader, sections, "faces", faces, checkedSectionBytes(vertexIndexCount, header.indexSizes.vertex, "vertex index"));
...
function createTextReader(reader, encoding) {
...
function skipVertices(reader, header, count) {
...
function skipVertex(reader, header, boneIndexSize) {
...
function isPostVertexSectionPlausible(reader, header) {
...
function readVariableSection(reader, sections, name, count, readPayload) {
...
function readFixedByteSection(reader, sections, name, count, byteLength) {
    const offset = reader.offset;
...
function skipMaterial(reader, readText, header) {
    readText();
    readText();
...
function skipBone(reader, readText, header) {
    readText();
    readText();
    reader.skip(12);
    ...
    if ((flags & PMX_BONE_FLAG_APPEND_ROTATE) !== 0 ||
...
function skipMorph(reader, readText, header) {
    readText();
    readText();
    reader.skip(1);
...
function skipDisplayFrame(reader, readText, header) {
    readText();
    readText();
    reader.skip(1);
...
function skipRigidBody(reader, readText, header) {
    readText();
    readText();
    skipIndex(reader, header.indexSizes.bone);
    reader.skip(1 + 2 + 1 + 36 + 20 + 1);
}
function skipJoint(reader, readText, header) {
    readText();
    readText();
    reader.skip(1);
    skipIndex(reader, header.indexSizes.rigidBody);
...
function skipSoftBody(reader, readText, header) {
    readText();
    readText();
    reader.skip(1);
    skipIndex(reader, header.indexSizes.material);
    ...
    reader.skip(checkedSectionBytes(pinnedCount, header.indexSizes.vertex, "soft body pinned vertex"));
}
function skipIndex(reader, size) {
    validateIndexSize("generic", size, [1, 2, 4]);
    reader.skip(size);
}
function skipVertexIndex(reader, size) {
...
function validateIndexSize(label, size, supported) {
