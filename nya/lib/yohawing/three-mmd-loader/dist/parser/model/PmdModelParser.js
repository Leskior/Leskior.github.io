...
export function parsePmd(bytes, options = {}) {
    ...
    const synthesizedIkChainBoneNames = [];
    ...
    for (let i = 0; i < morphCount; i++) {
        ...
        for (let j = 0; j < morphVertexCount; j++) {
            ...
            const vertexIndex = morphType === 0 ? morphVertexIndex : (baseMorphVertexIndices[morphVertexIndex] ?? -1);
    ...
    const { displayFrames, boneDisplayNameCount, boneDisplayFrameStart } = readPmdDisplayFrames(reader, morphs);
    const english = readOptionalEnglishBlock(reader, skeleton, morphs, displayFrames, boneDisplayFrameStart, boneDisplayNameCount);
    ...
    const unsupportedPmdBoneTypes = new Set(pmdBoneTypes.filter((boneType) => boneType === 3 || boneType > 9));
    if (unsupportedPmdBoneTypes.size > 0) {
        diagnostics.push({
            level: "warning",
            code: "PMD_BONE_TYPE_UNSUPPORTED",
            message: `Unsupported PMD bone types are present: ${Array.from(unsupportedPmdBoneTypes).join(", ")}; the current parser preserves them as basic rotatable bones.`
        });
    }
...
function readPmdDisplayFrames(reader, morphs) {
    const displayFrames = [];
...
function normalizePmdIndex(value) {
    return value === 0xffff ? -1 : value;
}
function createPmdBoneFlags(boneType, tailIndex) {
...
function applyPmdTwistAxes(skeleton, boneTypes) {
...
function createPmdIkChainBone(source, parentIndex) {
    return {
        name: `${source.name}+`,
        ...
        appendTransform: source.appendTransform === undefined ? undefined : { ...source.appendTransform },
        fixedAxis: source.fixedAxis === undefined ? undefined : [...source.fixedAxis],
        localAxis: source.localAxis === undefined
            ? undefined
            : {
...
function parsePmdMaterialTexture(textureName) {
...
function normalizePmdToonIndex(index) {
    return index >= 0 && index < 10 ? index : undefined;
}
function readOptionalEnglishBlock(reader, skeleton, morphs, displayFrames, boneDisplayFrameStart, boneDisplayNameCount) {
    if (reader.remaining < 1) {
        return { name: "", comment: "" };
    }
...
function readOptionalRigidBodies(reader) {
    if (reader.remaining < 4) {
        return [];
    }
...
function normalizePmdRigidBodyPositions(rigidBodies, skeleton) {
...
function readOptionalJoints(reader) {
    if (reader.remaining < 4) {
        return [];
    }
