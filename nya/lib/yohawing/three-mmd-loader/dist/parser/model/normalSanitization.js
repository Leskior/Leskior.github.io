const fallbackNormal = [0, 1, 0];
const normalEpsilon = 1e-12;
export function sanitizeNonFiniteModelNormals(positions, normals, indices, diagnostics) {
    ...
    for (let vertexIndex = 0; vertexIndex < vertexCount; vertexIndex += 1) {
        ...
        if (!Number.isFinite(normals[offset]) ||
...
function calculateFaceNormal(positions, a, b, c) {
    const ao = a * 3;
    const bo = b * 3;
    const co = c * 3;
...
function addNormal(accumulated, vertexIndex, normal) {
    const offset = vertexIndex * 3;
