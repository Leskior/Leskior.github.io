...
export function parsePmx(bytes, options = {}) {
    ...
    const { positions, normals, uvs, additionalUvs, skinIndices, skinWeights, edgeScale, sdef, sdefVertexCount, qdef, qdefVertexCount } = skipGeometry
    ...
    const softBodies = version >= 2.05 && reader.remaining >= 4 ? readSoftBodies(reader, readText, indexSizes) : [];
    const softBodyCount = softBodies.length;
    const diagnostics = createModelDiagnostics(materials, morphs, skeleton, rigidBodies, joints, displayFrames);
    ...
    if (softBodyCount > 0) {
        ...
        diagnostics.push(...createSoftBodyDiagnostics(softBodies, materialCount, vertexCount, rigidBodyCount));
    }
...
function readVerticesWithBoneIndexSizeFallback(reader, vertexCount, additionalUvCount, indexSizes) {
    ...
    for (const boneIndexSize of candidateBoneIndexSizes) {
        ...
        try {
            for (let i = 0; i < vertexCount; i++) {
                const weightType = readVertex(reader, additionalUvCount, candidateIndexSizes, buffers.positions, buffers.normals, buffers.uvs, buffers.additionalUvs, buffers.skinIndices, buffers.skinWeights, buffers.edgeScale, buffers.sdef, buffers.qdef, i);
                if (weightType === 3) {
...
function skipVerticesWithBoneIndexSizeFallback(reader, vertexCount, additionalUvCount, indexSizes) {
    const startOffset = reader.offset;
    const candidateBoneIndexSizes = uniqueNumbers([indexSizes.bone, 1, 2, 4]);
    let lastError;
    for (const boneIndexSize of candidateBoneIndexSizes) {
...
function readCount(reader, label, options = {}) {
    const count = reader.i32();
    validateCount(count, `PMX ${label}`, reader.remaining, options.remainingBytesPerEntry);
    return count;
}
function validateCount(count, label, remaining, remainingBytesPerEntry) {
...
function minimumPmxVertexByteLength(additionalUvCount, boneIndexSize) {
    return 3 * 4 + 3 * 4 + 2 * 4 + additionalUvCount * 4 * 4 + 1 + boneIndexSize + 4;
}
function createPmxVertexBuffers(vertexCount, additionalUvCount) {
    return {
        positions: new Float32Array(vertexCount * 3),
        normals: new Float32Array(vertexCount * 3),
        uvs: new Float32Array(vertexCount * 2),
        additionalUvs: Array.from({ length: additionalUvCount }, () => new Float32Array(vertexCount * 4)),
...
function createSkippedPmxVertexBuffers() {
    return {
...
function skipVertex(reader, additionalUvCount, sizes) {
...
function readVertex(reader, additionalUvCount, sizes, positions, normals, uvs, additionalUvs, skinIndices, skinWeights, edgeScale, sdef, qdef, index) {
...
function isPostVertexSectionPlausible(reader, indexSizes) {
...
function peekI32(reader, offset) {
...
function readSdefParameters(reader, sdef, vertexIndex, boneWeight0) {
    ...
    sdef.rw0.set([(c[0] + adjustedR0[0]) * 0.5, (c[1] + adjustedR0[1]) * 0.5, (c[2] + adjustedR0[2]) * 0.5], offset);
    sdef.rw1.set([(c[0] + adjustedR1[0]) * 0.5, (c[1] + adjustedR1[1]) * 0.5, (c[2] + adjustedR1[2]) * 0.5], offset);
}
function normalizeIndex(index) {
    return index < 0 ? 0 : index;
}
function readVertexIndex(reader, size) {
    switch (size) {
        case 1:
            return reader.u8();
        case 2:
...
function readMaterial(reader, readText, sizes, textures) {
    const name = readText();
    const englishName = readText();
    const diffuse = [
        reader.f32(),
    ...
    const specular = [reader.f32(), reader.f32(), reader.f32()];
    const specularPower = reader.f32();
    const ambient = [reader.f32(), reader.f32(), reader.f32()];
...
function parseMaterialFlags(bits) {
    return {
...
function readBone(reader, readText, sizes) {
    const name = readText();
    const englishName = readText();
    const position = [reader.f32(), reader.f32(), reader.f32()];
    ...
    const appendTransform = flags.appendRotate || flags.appendTranslate
        ? {
...
function readVec4(reader) {
    return [reader.f32(), reader.f32(), reader.f32(), reader.f32()];
}
function readIkLinks(reader, sizes) {
...
function readMorph(reader, readText, sizes) {
    const name = readText();
    const englishName = readText();
    reader.skip(1);
...
function readDisplayFrame(reader, readText, sizes) {
    const name = readText();
    const englishName = readText();
...
function readRigidBody(reader, readText, sizes) {
    return {
        name: readText(),
...
function readJoint(reader, readText, sizes) {
    return {
        name: readText(),
        englishName: readText(),
...
function readSoftBodies(reader, readText, sizes) {
...
function readSoftBodyAnchors(reader, sizes) {
...
function readSoftBodyPinnedVertexIndices(reader, sizes) {
...
function toSoftBodyAeroModel(value) {
    switch (value) {
        case 0:
            return "vertexPoint";
        case 1:
...
function createSoftBodyDiagnostics(softBodies, materialCount, vertexCount, rigidBodyCount) {
    const diagnostics = [];
    if (softBodies.some((softBody) => softBody.type === "unknown" || softBody.aeroModel === "unknown")) {
        diagnostics.push({
            level: "warning",
            code: "PMX_SOFT_BODY_TYPE_UNSUPPORTED",
            message: "Unknown PMX soft-body type or aero model values were parsed but cannot be simulated by the current runtime."
        });
    }
    for (const [softBodyIndex, softBody] of softBodies.entries()) {
        pushSoftBodyReferenceDiagnostic(diagnostics, softBody.materialIndex, materialCount, `soft body ${softBodyIndex} material`, "material");
        for (const [anchorIndex, anchor] of softBody.anchors.entries()) {
            pushSoftBodyReferenceDiagnostic(diagnostics, anchor.rigidBodyIndex, rigidBodyCount, `soft body ${softBodyIndex} anchor ${anchorIndex} rigid body`, "rigid body");
            pushSoftBodyReferenceDiagnostic(diagnostics, anchor.vertexIndex, vertexCount, `soft body ${softBodyIndex} anchor ${anchorIndex} vertex`, "vertex");
        }
        for (const [pinnedIndex, pinnedVertexIndex] of softBody.pinnedVertexIndices.entries()) {
            pushSoftBodyReferenceDiagnostic(diagnostics, pinnedVertexIndex, vertexCount, `soft body ${softBodyIndex} pinned vertex ${pinnedIndex}`, "vertex");
        }
    }
    return diagnostics;
}
function pushSoftBodyReferenceDiagnostic(diagnostics, index, count, owner, label) {
    if (index < 0) {
        return;
    }
    if (index >= count) {
...
function toRigidBodyShape(value) {
    switch (value) {
        case 0:
            return "sphere";
        case 1:
...
function toRigidBodyMode(value) {
    switch (value) {
        case 0:
            return "static";
        case 1:
