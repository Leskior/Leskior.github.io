...
export function parseVpd(input) {
...
export function vpdPoseToAnimation(pose, name = pose.metadata.modelFile) {
    const boneTracks = {};
    const morphTracks = {};
...
function createSingleBoneTrack(translation, rotation) {
    return {
        packed: "bone",
...
function createSingleMorphTrack(weight) {
    return {
        packed: "morph",
        frames: new Uint32Array([0]),
...
function readMorphPoses(text, startIndex) {
    const morphs = {};
    const blockPattern = /Morph\d+\s*\{\s*([^\r\n]+)\s*\r?\n\s*(?:(?:\/\/)[^\r\n]*\r?\n\s*)?([^;]+);/g;
    blockPattern.lastIndex = startIndex;
    for (const match of text.matchAll(blockPattern)) {
        const name = (match[1] ?? "").trim();
