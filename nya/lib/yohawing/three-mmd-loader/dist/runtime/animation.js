...
export function applyMmdAnimation(mesh, animation, restTransforms, preAppendTransforms, scratch, frame) {
    if (!mesh || !animation) {
        return;
    }
    ...
    for (let index = 0; index < bones.length; index += 1) {
        ...
        if (sampled.hasPhysicsToggle) {
            ...
            bonePhysicsToggles[typeof userData.mmdBoneName === "string" ? userData.mmdBoneName : bone.name] = sampled.physicsToggle;
        ...
        bone.position.set(restPosition.x + sampled.translationX, restPosition.y + sampled.translationY, restPosition.z - sampled.translationZ);
        bone.quaternion.set(-sampled.rotationX, -sampled.rotationY, sampled.rotationZ, sampled.rotationW);
    }
...
function isMmdAnimation(value) {
    return (typeof value === "object" && value !== null && "boneTracks" in value && "morphTracks" in value);
}
function findBoneTrack(animation, bone) {
    const userData = readMmdBoneUserData(bone);
...
function sampleBoneTrack(frames, frame) {
    if (!frames) {
        return undefined;
    }
    return samplePackedBoneTrack(frames, frame);
}
function sampleBoneTrackInto(track, frame, target) {
    if (!track || !samplePackedBoneTrackInto(track, frame, target)) {
        return undefined;
    }
    return target;
}
function sampleMorphTrack(frames, frame) {
    return samplePackedMorphTrack(frames, frame);
}
export function sampleMmdCameraTrack(frames, frame) {
...
export function sampleMmdCameraTrackInto(frames, frame, target, hint) {
    if (frames.length === 0) {
        return undefined;
    }
...
export function sampleMmdLightTrack(frames, frame) {
    return sampleMmdLightTrackInto(frames, frame, {
        color: [0, 0, 0],
        direction: [0, 0, 0]
    });
}
export function sampleMmdLightTrackInto(frames, frame, target) {
...
export function sampleMmdSelfShadowTrack(frames, frame) {
    if (frames.length === 0) {
        return undefined;
    }
...
export function sampleMmdSelfShadowTrackInto(frames, frame, target, hint) {
    if (frames.length === 0) {
        return undefined;
    }
    let index = hint?.index ?? 0;
...
function sampleFramePair(frames, frame) {
    if (frames.length === 0) {
        return undefined;
    }
    if (frame < frames[0].frame) {
...
function expandMorphWeight(morphs, weights, morphIndex, weight, visited) {
...
function applyBoneMorphs(mesh, morphs, weights, scratchQuaternion) {
    if (morphs.length === 0) {
        return;
    }
    for (let morphIndex = 0; morphIndex < morphs.length; morphIndex += 1) {
        ...
        for (const offset of morph.boneOffsets ?? []) {
            ...
            scratchQuaternion.set(-offset.rotation[0], -offset.rotation[1], offset.rotation[2], offset.rotation[3]);
            bone.quaternion.premultiply(weightedThreeQuaternion(scratchQuaternion, weight, scratchQuaternion));
        }
    }
}
export function preparePreAppendTransforms(bones, preAppendTransforms) {
...
function copyPreAppendTransforms(bones, preAppendTransforms) {
    preAppendTransforms.length = bones.length;
    for (let index = 0; index < bones.length; index += 1) {
        const bone = bones[index];
...
function samplePackedBoneTrackInto(track, frame, target) {
...
function readPackedBoneFrameInto(track, index, frame, target) {
...
function readInterpolatedPackedBoneFrameInto(track, previousIndex, nextIndex, frame, t, target) {
    ...
    target.frame = frame;
    target.translationX = lerp(track.translations[previousTranslationOffset] ?? 0, track.translations[nextTranslationOffset] ?? 0, interpolatePackedCurve(track.interpolations, interpolationOffset, t));
    target.translationY = lerp(track.translations[previousTranslationOffset + 1] ?? 0, track.translations[nextTranslationOffset + 1] ?? 0, interpolatePackedCurve(track.interpolations, interpolationOffset + 4, t));
    target.translationZ = lerp(track.translations[previousTranslationOffset + 2] ?? 0, track.translations[nextTranslationOffset + 2] ?? 0, interpolatePackedCurve(track.interpolations, interpolationOffset + 8, t));
    slerpPackedRotationInto(track, previousIndex, nextIndex, interpolatePackedCurve(track.interpolations, interpolationOffset + 12, t), target);
    const physicsToggle = track.physicsToggles[previousIndex] ?? -1;
    target.hasPhysicsToggle = physicsToggle >= 0;
    target.physicsToggle = physicsToggle;
}
function interpolatePackedCurve(values, offset, x) {
...
function slerpPackedRotationInto(track, previousIndex, nextIndex, t, target) {
    ...
    if (cos > 0.9995) {
        normalizeSampleRotationInto(lerp(ax, bx, t), lerp(ay, by, t), lerp(az, bz, t), lerp(aw, bw, t), target);
        return;
    }
...
function normalizeSampleRotationInto(x, y, z, w, target) {
...
function samplePackedBoneTrack(track, frame) {
    const frames = track.frames;
    if (frames.length === 0) {
        return undefined;
    }
...
function samplePackedMorphTrack(track, frame) {
    const frames = track.frames;
    if (frames.length === 0) {
        return 0;
    }
...
function readPackedBoneFrame(track, index, frame) {
    const translationOffset = index * 3;
    const rotationOffset = index * 4;
    const physicsToggle = track.physicsToggles[index] ?? -1;
    const result = {
...
function readPackedBoneInterpolation(track, index) {
...
function readPackedCurve(values, offset) {
    return [
        values[offset] ?? 0,
        values[offset + 1] ?? 0,
        values[offset + 2] ?? 0,
...
function clearNumberRecord(record) {
    for (const key in record) {
        delete record[key];
    }
}
export { findBoneTrack, isMmdAnimation, sampleBoneTrack, sampleFramePair, sampleMorphTrack };
function interpolationRatio(previousFrame, nextFrame, frame) {
