...
export function applyMmdLightStateToThreeDirectionalLight(light, state, options = {}) {
    if (!state) {
        return light;
    }
    const colorScale = options.colorScale ?? 1;
    light.color.setRGB(state.color[0] * colorScale, state.color[1] * colorScale, state.color[2] * colorScale);
