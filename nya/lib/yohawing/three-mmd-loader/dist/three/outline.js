...
import { computeMmdMaterialRenderOrder, mmdMaterialCastsShadow, mmdMaterialCastsSelfShadow, mmdMaterialSuppressesColorAtAlpha } from "./material/material-metadata.js";
...
export function createMmdOutlineMeshes(model, options = {}) {
    ...
    for (let materialIndex = 0; materialIndex < materialInfos.length; materialIndex += 1) {
        ...
        const outlineMaterial = createMmdOutlineMaterial(materialInfo, materialIndex, options, hasVertexEdgeScale);
...
function createMmdOutlineMaterial(material, index, options, hasVertexEdgeScale) {
...
export function attachMmdOutlineExpansion(material, outlineWidth, hasVertexEdgeScale) {
    ...
    material.customProgramCacheKey = () => `${previousProgramCacheKey()}-yw-mmd-outline-expansion${hasVertexEdgeScale ? "-edge-scale" : ""}`;
    material.needsUpdate = true;
}
function attachMmdPmxOutlineExpansion(material, outlineWidth, hasVertexEdgeScale) {
    const previousOnBeforeCompile = material.onBeforeCompile.bind(material);
    const previousProgramCacheKey = material.customProgramCacheKey.bind(material);
    material.onBeforeCompile = (shader, renderer) => {
        previousOnBeforeCompile(shader, renderer);
        ...
        material.userData.mmdOutlineUpdateViewport = (activeRenderer) => {
    ...
    material.customProgramCacheKey = () => `${previousProgramCacheKey()}-yw-mmd-pmx-outline-expansion${hasVertexEdgeScale ? "-edge-scale" : ""}`;
    material.needsUpdate = true;
}
function attachMmdOutlineViewportUpdate(outline) {
    outline.onBeforeRender = (renderer) => {
        syncMmdOutlineViewport(outline.material, renderer);
    };
}
function syncMmdOutlineViewport(materials, renderer) {
...
function syncMmdOutlineMaterialViewport(material, renderer) {
    const updateViewport = material.userData.mmdOutlineUpdateViewport;
    updateViewport?.(renderer);
}
function updateMmdOutlineViewport(renderer, target, currentViewport, pixelRatioUniform) {
...
function mmdOutlineExpansionWidth(material, options, hasEdge) {
    const edgeSize = hasEdge ? material.edgeSize : options.forceFallback ? 0.5 : 0;
    return Math.max(edgeSize, 0);
}
function mmdOutlinePolygonOffsetFactor(outlineWidth) {
    return 1 + 2 * Math.max(outlineWidth, 0);
}
export function createMmdMaterialRenderOrderMeshes(model, options = {}) {
    ...
    for (const entry of renderOrder) {
        ...
        const geometry = createMmdMaterialProxyGeometry(model.mesh.geometry, group);
        const mesh = new THREE.SkinnedMesh(geometry, options.shadowOnly ? createShadowOnlyMaterial(material) : material);
...
function createShadowOnlyMaterial(source) {
...
function mmdMaterialRenderOrderEntries(model) {
    const sourceMaterials = Array.isArray(model.mesh.material)
        ? model.mesh.material
        : [model.mesh.material];
    return (model.mesh.userData.mmdMaterialRenderOrder ??
        computeMmdMaterialRenderOrder(sourceMaterials.map((material, materialIndex) => ({
            materialIndex,
            transparencyMode: material.userData.mmdMaterial?.transparencyMode ??
                "opaque"
        }))));
}
function mmdMaterialRenderOrderMap(renderOrder) {
    return new Map(renderOrder.map((entry) => [entry.materialIndex, entry.renderOrder]));
}
function createMmdMaterialProxyGeometry(source, group) {
...
export function syncMmdOutlineMaterialStates(materials, states) {
    const materialList = Array.isArray(materials) ? materials : [materials];
    materialList.forEach((material, fallbackIndex) => {
        const metadata = material.userData.mmdOutlineMaterial;
        ...
        if ("color" in material && material.color instanceof THREE.Color) {
            material.color.setRGB(clampColor(state.edgeColor[0]), clampColor(state.edgeColor[1]), clampColor(state.edgeColor[2]));
        }
        ...
        const shader = material.userData.mmdOutlineShader;
...
function mmdOutlineRuntimeWidth(edgeSize, fallback) {
