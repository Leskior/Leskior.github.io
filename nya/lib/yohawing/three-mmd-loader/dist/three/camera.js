...
export function applyMmdCameraStateToThreeCamera(camera, state, options) {
    const activeCamera = state.perspective || !options?.orthographicCamera ? camera : options.orthographicCamera;
    ...
    lookAt.set(activeCamera.position.x + lookAt.x, activeCamera.position.y + lookAt.y, activeCamera.position.z - lookAt.z);
    ...
    if (activeCamera instanceof THREE.PerspectiveCamera) {
        ...
        const height = Math.max(2 * Math.abs(state.distance) * Math.tan(THREE.MathUtils.degToRad(fov) / 2), options?.minOrthographicHeight ?? 1e-3);
