...
export function createThreeBufferGeometry(buffers, materials = [], morphs = [], options = {}) {
    ...
    const indices = buffers.indices instanceof Uint32Array
    ...
    if (buffers.sdef) {
        geometry.setAttribute("matricesSdefEnabled", new THREE.BufferAttribute(buffers.sdef.enabled.slice(), 1));
        geometry.setAttribute("matricesSdefC", new THREE.BufferAttribute(createThreeVec3Buffer(buffers.sdef.c), 3));
        geometry.setAttribute("matricesSdefR0", new THREE.BufferAttribute(createThreeVec3Buffer(buffers.sdef.r0), 3));
        geometry.setAttribute("matricesSdefR1", new THREE.BufferAttribute(createThreeVec3Buffer(buffers.sdef.r1), 3));
        geometry.setAttribute("matricesSdefRW0", new THREE.BufferAttribute(createThreeVec3Buffer(buffers.sdef.rw0), 3));
        geometry.setAttribute("matricesSdefRW1", new THREE.BufferAttribute(createThreeVec3Buffer(buffers.sdef.rw1), 3));
        geometry.userData.mmdSdef = { vertexCount: buffers.sdef.c.length / 3 };
    }
    if (buffers.qdef) {
        geometry.setAttribute("matricesQdefEnabled", new THREE.BufferAttribute(buffers.qdef.enabled.slice(), 1));
    ...
    if (morphs.length > 0) {
        ...
        if (options.morphAttributes ?? true) {
            ...
            if (morphs.some(hasPositionMorphOffsets)) {
                geometry.morphAttributes.position = createMorphAttributes(morphs, buffers.positions.length, 3, (morph, length) => createThreeMorphPositionOffsets(length, morph));
            }
            if (morphs.some(hasUvMorphOffsets)) {
                morphAttributes.uv = createMorphAttributes(morphs, buffers.uvs.length, 2, (morph, length) => createThreeMorphUvOffsets(length, morph));
            }
            additionalUvs.forEach((additionalUv, index) => {
                if (morphs.some((morph) => hasAdditionalUvMorphOffsets(morph, index))) {
                    morphAttributes[`uv${index + 1}`] = createMorphAttributes(morphs, additionalUv.length, 4, (morph, length) => createThreeAdditionalMorphUvOffsets(length, index, morph));
                }
            });
        }
    }
...
export function createThreeMorphSplitGeometries(buffers, materials = [], morphs = []) {
    validateGeometryInput(buffers, materials, morphs);
    ...
    for (const group of groups) {
        ...
        const geometry = createThreeBufferGeometry(split.buffers, [{ faceCount: group.count / 3, materialIndex: group.materialIndex }], splitMorphs.morphs);
...
function createMmdRenderOrderGroups(buffers, materials) {
    ...
    const renderOrderByMaterial = new Map(computeMmdMaterialRenderOrder(materials.map((material, fallbackIndex) => ({
...
function createMaterialFaceCountGroups(materials) {
    let groupStart = 0;
...
function createMaterialSplitBuffers(buffers, group) {
    ...
    const indexArray = localVertices.length > 65535
    ...
    const splitBuffers = {
        ...
        additionalUvs: buffers.additionalUvs?.map((additionalUv) => copySplitFloatAttribute(additionalUv, 4, localToSource)),
        indices: indexArray,
...
function copySplitFloatAttribute(source, itemSize, localToSource) {
...
function copySplitUint16Attribute(source, itemSize, localToSource) {
...
function createMaterialSplitMorphs(morphs, sourceToLocal, localToSource) {
    const splitMorphs = [];
    const morphTargetIndices = morphs.length > 65535 ? new Uint32Array(morphs.length) : new Uint16Array(morphs.length);
    let splitIndex = 0;
...
function splitVertexOffsets(morph, sourceToLocal, localToSource) {
...
function splitUvOffsets(morph, sourceToLocal, localToSource) {
...
function splitAdditionalUvOffsets(morph, sourceToLocal, localToSource) {
    const offsets = [];
...
function validateGeometryInput(buffers, materials, morphs) {
...
function validateBaseBuffers(buffers) {
...
function validateMaterialGroups(buffers, materials) {
    buffers.materialGroups?.forEach((group, index) => {
        ...
        if (group.start % 3 !== 0 ||
            ...
            throw new RangeError(`THREE_MMD_GEOMETRY_MATERIAL_GROUP_RANGE_INVALID:${index}:${group.start}:${group.count}`);
        }
    });
    ...
    if (!buffers.materialGroups?.length &&
        ...
        throw new RangeError(`THREE_MMD_GEOMETRY_MATERIAL_FACE_COUNT_MISMATCH:${derivedGroupCount}:${buffers.indices.length}`);
    }
}
function validateMaterialGroupCoverage(groups, indexCount) {
...
function validateMorphs(morphs, vertexCount, additionalUvCount) {
    morphs.forEach((morph, morphIndex) => {
        ...
        if (sparse) {
            ...
            morph.vertexOffsets?.forEach((offset, offsetIndex) => {
                validateVertexIndex(`MORPH_VERTEX:${morphIndex}:${offsetIndex}`, offset.vertexIndex, vertexCount);
                validateTuple(`THREE_MMD_GEOMETRY_MORPH_POSITION_INVALID:${morphIndex}:${offsetIndex}`, offset.position, 3);
            });
        }
        validateDenseMorphBuffer(`THREE_MMD_GEOMETRY_MORPH_POSITION_DENSE_INVALID:${morphIndex}`, morph.densePositionOffsets, vertexCount * 3);
        morph.uvOffsets?.forEach((offset, offsetIndex) => {
            ...
            validateTuple(`THREE_MMD_GEOMETRY_MORPH_UV_INVALID:${morphIndex}:${offsetIndex}`, offset.uv, 2);
        });
        validateDenseMorphBuffer(`THREE_MMD_GEOMETRY_MORPH_UV_DENSE_INVALID:${morphIndex}`, morph.denseUvOffsets, vertexCount * 2);
        morph.additionalUvOffsets?.forEach((offset, offsetIndex) => {
            validateVertexIndex(`MORPH_ADDITIONAL_UV:${morphIndex}:${offsetIndex}`, offset.vertexIndex, vertexCount);
            if (!Number.isInteger(offset.uvIndex) ||
                offset.uvIndex < 0 ||
                offset.uvIndex >= additionalUvCount) {
                throw new RangeError(`THREE_MMD_GEOMETRY_MORPH_ADDITIONAL_UV_INDEX_INVALID:${morphIndex}:${offsetIndex}:${offset.uvIndex}`);
            }
            validateTuple(`THREE_MMD_GEOMETRY_MORPH_ADDITIONAL_UV_INVALID:${morphIndex}:${offsetIndex}`, offset.uv, 4);
        });
        morph.denseAdditionalUvOffsets?.forEach((offsets, uvIndex) => {
            if (uvIndex >= additionalUvCount) {
                throw new RangeError(`THREE_MMD_GEOMETRY_MORPH_ADDITIONAL_UV_DENSE_INDEX_INVALID:${morphIndex}:${uvIndex}`);
            }
            validateDenseMorphBuffer(`THREE_MMD_GEOMETRY_MORPH_ADDITIONAL_UV_DENSE_INVALID:${morphIndex}:${uvIndex}`, offsets, vertexCount * 4);
        });
    });
}
function validateBufferLength(name, actual, expected) {
    if (actual !== expected) {
        throw new RangeError(`THREE_MMD_GEOMETRY_${name}_LENGTH_INVALID:${actual}:${expected}`);
    }
}
function validateFiniteBuffer(name, values) {
...
function validateIndexBuffer(indices, vertexCount) {
...
function validateDenseMorphBuffer(name, values, expectedLength) {
    if (!values) {
        return;
    }
...
function createThreeVec3Buffer(values) {
...
function createMorphAttributes(morphs, length, itemSize, createOffsets) {
...
function hasUvMorphOffsets(morph) {
    return !!morph.uvOffsets?.length || !!morph.denseUvOffsets;
}
function hasPositionMorphOffsets(morph) {
    return ((getDenseMorphProvider(morph)?.sparsePositionOffsets?.count ?? 0) > 0 ||
        !!morph.vertexOffsets?.length ||
        !!morph.densePositionOffsets);
}
function hasAdditionalUvMorphOffsets(morph, uvIndex) {
    return (!!morph.denseAdditionalUvOffsets?.[uvIndex] ||
        !!morph.additionalUvOffsets?.some((offset) => offset.uvIndex === uvIndex));
}
function hasGeometryMorphOffsets(morph) {
    return ((getDenseMorphProvider(morph)?.sparsePositionOffsets?.count ?? 0) > 0 ||
        !!morph.vertexOffsets?.length ||
        !!morph.densePositionOffsets ||
...
function createThreeMorphPositionOffsets(positionLength, morph) {
...
function createThreeMorphUvOffsets(uvLength, morph) {
...
function createThreeAdditionalMorphUvOffsets(uvLength, uvIndex, morph) {
    const providerOffsets = getDenseMorphProvider(morph)?.createAdditionalUvOffsets(uvIndex, uvLength / 4);
    if (providerOffsets) {
        return providerOffsets;
    }
...
function getDenseMorphProvider(morph) {
    return morph[denseMorphProviderSymbol];
}
function validateSparsePositionMorphOffsets(sparse, morphIndex, vertexCount) {
    const { vertexIndices, positions, start, count } = sparse;
    ...
    for (let offsetIndex = 0; offsetIndex < count; offsetIndex += 1) {
        const sourceIndex = start + offsetIndex;
        validateVertexIndex(`MORPH_VERTEX:${morphIndex}:${offsetIndex}`, vertexIndices[sourceIndex] ?? 0, vertexCount);
        const positionIndex = sourceIndex * 3;
        if (!Number.isFinite(positions[positionIndex]) ||
            !Number.isFinite(positions[positionIndex + 1]) ||
            !Number.isFinite(positions[positionIndex + 2])) {
            throw new RangeError(`THREE_MMD_GEOMETRY_MORPH_POSITION_INVALID:${morphIndex}:${offsetIndex}`);
        }
    }
}
