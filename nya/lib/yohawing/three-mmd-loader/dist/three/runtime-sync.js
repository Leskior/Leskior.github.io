...
const scratchWorldMatrices = [];
const scratchLocalMatrix = new THREE.Matrix4();
export function mmdWorldMatrixToThree(matrices, index = 0) {
...
export function syncThreeMmdRuntimeToMesh(model, mesh, runtime, outlineMeshes, renderOrderMeshes) {
    syncThreeMmdRuntimeToMeshInternal(model, mesh, runtime, outlineMeshes, renderOrderMeshes);
}
export function syncThreeMmdRuntimeToModel(model, target, runtime) {
    syncThreeMmdRuntimeToMeshInternal(model, target.mesh, runtime, target.outlineMeshes, target.renderOrderMeshes);
}
function syncThreeMmdRuntimeToMeshInternal(model, mesh, runtime, outlineMeshes, renderOrderMeshes) {
...
function syncRenderOrderProxyMaterialState(mesh, materialStates, fallbackIndex) {
    const metadata = mesh.userData.mmdMaterialRenderProxy;
...
function syncRuntimeMorphWeights(mesh, weights) {
...
function syncRuntimeBoneTransforms(model, mesh, coreMatrices) {
    ...
    for (let index = 0; index < bones.length; index += 1) {
        ...
        const localMatrix = parentIndex >= 0
