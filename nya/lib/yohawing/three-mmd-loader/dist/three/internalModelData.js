...
export function createLoaderMmdModelData(input) {
    validateLoaderMmdModelData(input);
    return input;
}
export function createLoaderMmdMetadata(metadata) {
    return {
...
export function validateLoaderMmdModelData(modelData) {
...
function sanitizeGeometryNormals(modelData) {
    ...
    if (positions instanceof Float32Array &&
        ...
        sanitizeNonFiniteModelNormals(positions, normals, indices, modelData.metadata.diagnostics);
    }
}
...
function validateMorphs(morphs) {
    morphs.forEach((morph, morphIndex) => {
        ...
        if (sparse) {
            ...
            for (let offsetIndex = 0; offsetIndex < count; offsetIndex += 1) {
                ...
                validateNonNegativeInteger(`MORPH_VERTEX:${morphIndex}:${offsetIndex}`, vertexIndices[sourceIndex] ?? 0);
...
function validateSkeletonShape(skeleton) {
...
function validateFloat32Buffer(name, buffer, itemSize, expectedItems) {
    ...
    if (expectedItems !== undefined && buffer.length !== expectedItems * itemSize) {
        throw new RangeError(`LOADER_MMD_MODEL_${name}_COUNT_INVALID:${buffer.length}:${expectedItems * itemSize}`);
    }
...
function validateUint16Buffer(name, buffer, itemSize, expectedItems) {
...
function validateNumberTuple(name, values, expectedLength) {
