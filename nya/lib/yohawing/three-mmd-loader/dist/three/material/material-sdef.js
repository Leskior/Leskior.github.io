...
export function attachMmdSdefSkinning(material) {
    ...
    material.onBeforeCompile = (shader, renderer) => {
        ...
        shader.vertexShader = shader.vertexShader.replace("#include <skinning_pars_vertex>", ["#include <skinning_pars_vertex>", MMD_SDEF_SKINNING_DECLARATION].join("\n"));
        shader.vertexShader = shader.vertexShader.replace("#include <skinnormal_vertex>", MMD_SDEF_SKINNING_NORMAL);
        shader.vertexShader = shader.vertexShader.replace("#include <skinning_vertex>", MMD_SDEF_SKINNING_VERTEX);
    };
    ...
    const matWithDefaults = material;
...
export function computeMmdSdefSkinnedPosition(input) {
    ...
    const positionOffset = input.sdefRW0
        ...
        .add(input.sdefRW1.clone().applyMatrix4(input.boneMatrices[1]).multiplyScalar(input.skinWeights[1]));
    return skinVertex
...
function createWeightedMatrix(matrices, weights) {
