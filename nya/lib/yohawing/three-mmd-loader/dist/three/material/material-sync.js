...
export function syncMmdMaterialStates(materials, states) {
    ...
    materialList.forEach((material, index) => {
        ...
        if (materialColor instanceof THREE.Color) {
            materialColor.setRGB(clampColor(state.diffuse[0]), clampColor(state.diffuse[1]), clampColor(state.diffuse[2]));
        }
        ...
        const sphereShader = material.userData.mmdSphereShader;
        const materialFactorShader = material.userData.mmdMaterialFactorShader;
        const emissive = material.emissive;
        if (!material.userData.mmdMaterialFactors?.shaderApplied &&
            !materialFactorShader &&
            emissive instanceof THREE.Color) {
            emissive.setRGB(clampColor(state.ambient[0]), clampColor(state.ambient[1]), clampColor(state.ambient[2]));
        }
        ...
        if (diffuseColorUniform instanceof THREE.Color) {
            diffuseColorUniform.setRGB(clampColor(state.diffuse[0]), clampColor(state.diffuse[1]), clampColor(state.diffuse[2]));
        }
        const textureFactor = materialFactorShader?.uniforms?.mmdTextureFactor?.value;
        if (textureFactor instanceof THREE.Vector4) {
            textureFactor.set(state.textureFactor[0], state.textureFactor[1], state.textureFactor[2], state.textureFactor[3]);
        }
        const sphereFactor = sphereShader?.uniforms?.mmdSphereFactor?.value;
        if (sphereFactor instanceof THREE.Vector4) {
            sphereFactor.set(state.sphereTextureFactor[0], state.sphereTextureFactor[1], state.sphereTextureFactor[2], state.sphereTextureFactor[3]);
        }
        const toonFactor = materialFactorShader?.uniforms?.mmdToonTextureFactor?.value;
        if (toonFactor instanceof THREE.Vector4) {
            toonFactor.set(state.toonTextureFactor[0], state.toonTextureFactor[1], state.toonTextureFactor[2], state.toonTextureFactor[3]);
        }
        const materialAmbient = materialFactorShader?.uniforms?.mmdMaterialAmbient?.value;
        if (materialAmbient instanceof THREE.Color) {
            materialAmbient.setRGB(clampColor(state.ambient[0]), clampColor(state.ambient[1]), clampColor(state.ambient[2]));
        }
        const specularColor = materialFactorShader?.uniforms?.mmdSpecularColor?.value;
        if (specularColor instanceof THREE.Color) {
            specularColor.setRGB(clampColor(state.specular[0]), clampColor(state.specular[1]), clampColor(state.specular[2]));
        }
...
export function syncMmdSpecularDirection(material, lightDirection) {
    const materialList = Array.isArray(material) ? material : [material];
    const directionSource = mmdLightDirectionSource(lightDirection);
    const directColor = lightDirection instanceof THREE.Light
        ? [
    ...
    materialList.forEach((mat) => {
        ...
        const shader = mat.userData.mmdMaterialFactorShader;
...
function getMmdLightUniformState(material) {
    const existing = material.userData.mmdLightUniformState;
    if (existing &&
...
function mmdLightDirectionSource(lightDirection) {
