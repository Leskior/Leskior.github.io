...
function matrixToDualQuaternion(matrix) {
    ...
    //   z: tz*qw + tx*qy - ty*qx
    //   w: -tx*qx - ty*qy - tz*qz
    const dual = new THREE.Quaternion(0.5 * (t.x * real.w + t.y * real.z - t.z * real.y), 0.5 * (t.y * real.w + t.z * real.x - t.x * real.z), 0.5 * (t.z * real.w + t.x * real.y - t.y * real.x), 0.5 * (-t.x * real.x - t.y * real.y - t.z * real.z));
    return { real, dual };
}
/**
 ...
 */
function blendDualQuaternions(dqs, weights) {
    ...
    const len = Math.sqrt(blendedReal.x * blendedReal.x +
        blendedReal.y * blendedReal.y +
        blendedReal.z * blendedReal.z +
        blendedReal.w * blendedReal.w) || 1e-6;
    blendedReal.x /= len;
    blendedReal.y /= len;
    blendedReal.z /= len;
    blendedReal.w /= len;
 ...
 */
function applyDualQuaternion(dq, p) {
    const q = dq.real;
    const qd = dq.dual;
 ...
 */
export function computeQdefSkinnedPosition(input) {
    ...
    const dqs = input.boneMatrices.map(matrixToDualQuaternion);
 ...
 */
export function computeQdefSkinnedNormal(input) {
    const dqs = input.boneMatrices.map(matrixToDualQuaternion);
    const blended = blendDualQuaternions(dqs, input.skinWeights);
