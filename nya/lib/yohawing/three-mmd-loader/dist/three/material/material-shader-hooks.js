...
const MMD_DEFAULT_LIGHT_TRAVEL_DIRECTION = [-0.5, -1.0, -1.0];
...
const DIRECTIONAL_SHADOW_COLOR_MULTIPLY = "directLight.color *= ( directLight.visible && receiveShadow ) ? getShadow( directionalShadowMap[ i ], directionalLightShadow.shadowMapSize, directionalLightShadow.shadowIntensity, directionalLightShadow.shadowBias, directionalLightShadow.shadowRadius, vDirectionalShadowCoord[ i ] ) : 1.0;";
const MMD_DIRECTIONAL_SELF_SHADOW_FACTOR = "ywMmdToonShadowFactor = min( ywMmdToonShadowFactor, ( mmdSelfShadowReceive > 0.5 && directLight.visible && receiveShadow ) ? getShadow( directionalShadowMap[ i ], directionalLightShadow.shadowMapSize, directionalLightShadow.shadowIntensity, directionalLightShadow.shadowBias, directionalLightShadow.shadowRadius, vDirectionalShadowCoord[ i ] ) : 1.0 );";
/**
...
export function attachMmdMaterialFactors(material) {
    ...
    material.onBeforeCompile = (shader, renderer) => {
        ...
        const lightUniformState = material.userData.mmdLightUniformState;
        shader.uniforms.mmdLightDirection = {
            value: new THREE.Vector3(-MMD_DEFAULT_LIGHT_TRAVEL_DIRECTION[0], -MMD_DEFAULT_LIGHT_TRAVEL_DIRECTION[1], -MMD_DEFAULT_LIGHT_TRAVEL_DIRECTION[2]).normalize()
        };
        shader.uniforms.mmdLightColor = {
            value: new THREE.Color(MMD_DEFAULT_LIGHT_COLOR, MMD_DEFAULT_LIGHT_COLOR, MMD_DEFAULT_LIGHT_COLOR)
        };
        ...
        if (lightUniformState) {
            shader.uniforms.mmdLightDirection.value.set(lightUniformState.direction[0], lightUniformState.direction[1], lightUniformState.direction[2]);
            if (lightUniformState.directColor) {
                shader.uniforms.mmdLightColor.value.setRGB(lightUniformState.directColor[0], lightUniformState.directColor[1], lightUniformState.directColor[2]);
            }
        }
        const materialState = material.userData.mmdMaterialState;
        if (materialState) {
            shader.uniforms.mmdDiffuseColor.value.setRGB(clampColor(materialState.diffuse[0]), clampColor(materialState.diffuse[1]), clampColor(materialState.diffuse[2]));
            shader.uniforms.mmdTextureFactor.value.set(materialState.textureFactor[0], materialState.textureFactor[1], materialState.textureFactor[2], materialState.textureFactor[3]);
            shader.uniforms.mmdToonTextureFactor.value.set(materialState.toonTextureFactor[0], materialState.toonTextureFactor[1], materialState.toonTextureFactor[2], materialState.toonTextureFactor[3]);
            shader.uniforms.mmdMaterialAmbient.value.setRGB(clampColor(materialState.ambient[0]), clampColor(materialState.ambient[1]), clampColor(materialState.ambient[2]));
            shader.uniforms.mmdSpecularColor.value.setRGB(clampColor(materialState.specular[0]), clampColor(materialState.specular[1]), clampColor(materialState.specular[2]));
...
function injectMmdShading(fragmentShader) {
    let shader = fragmentShader;
    // Declare uniforms + helpers.
    shader = shader.replace("#include <map_pars_fragment>", ["#include <map_pars_fragment>", MMD_FRAGMENT_PARS].join("\n"));
    ...
    shader = shader.replace("#include <map_fragment>", [
        "#ifdef USE_MAP",
    ...
    shader = shader.replace("#include <lights_fragment_begin>", mmdLightsFragmentBegin(MMD_DIRECTIONAL_SELF_SHADOW_FACTOR));
    return shader;
}
function mmdLightsFragmentBegin(shadowReplacement) {
    return THREE.ShaderChunk.lights_fragment_begin
        .replace(DIRECTIONAL_LIGHT_INFO_CALL, [DIRECTIONAL_LIGHT_INFO_CALL].join("\n"))
        .replace(DIRECTIONAL_SHADOW_COLOR_MULTIPLY, shadowReplacement);
}
function readMaterialRawDiffuse(material) {
    const diffuse = material.userData.mmdMaterial?.diffuse;
    if (diffuse) {
...
function readMaterialRawVec3(material, key, fallback) {
    const value = material.userData.mmdMaterial?.[key];
    if (value) {
        return [clampColor(value[0]), clampColor(value[1]), clampColor(value[2])];
    }
    return fallback;
}
function readMaterialRawSpecularPower(material) {
    const value = material.userData.mmdMaterial?.specularPower;
    return Number.isFinite(value) ? value : 0;
}
function materialReceivesMmdSelfShadow(material) {
    const flags = material.userData.mmdMaterial?.flags;
    return flags?.selfShadow !== false;
}
export function materialHasTextureMap(material) {
    return "map" in material && material.map instanceof THREE.Texture;
}
export function attachMmdSphereTexture(material, sphereMode, texture) {
    ...
    material.onBeforeCompile = (shader, renderer) => {
        ...
        if (materialState?.sphereTextureFactor) {
            shader.uniforms.mmdSphereFactor.value.set(materialState.sphereTextureFactor[0], materialState.sphereTextureFactor[1], materialState.sphereTextureFactor[2], materialState.sphereTextureFactor[3]);
        }
    ...
    material.customProgramCacheKey = () => `${previousProgramCacheKey()}-yw-mmd-sphere-${sphereMode}`;
    material.needsUpdate = true;
}
export function mmdSphereModeToUniform(sphereMode) {
    switch (sphereMode) {
        case "multiply":
            return 1;
        case "add":
